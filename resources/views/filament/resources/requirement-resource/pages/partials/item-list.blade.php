@php
    ...
    if ($isEditMode && $currentRecord->relationLoaded('requirementLists')) {
        $initialItems = $currentRecord->requirementLists->map(function ($rl) {
            return [
                'list_id' => $rl->id,
                'id' => $rl->item_id,
...
<div class="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950 flex flex-col h-full"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
            ...
            <div class="sm:col-span-6 relative">
                ...
                <input type="text" x-model="search" @input.debounce.400ms="refreshItems(1)"
                    ...
                    class="shadcn-input !h-8 !text-xs !pl-8 !pr-7" />
                <div class="absolute inset-y-0 right-0 flex items-center pr-2" x-show="search">
                    <button type="button" @click="search = ''; refreshItems(1)" class="text-zinc-400 hover:text-zinc-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="sm:col-span-3">
                <select x-model="selectedSubcategoryId" @change="refreshItems(1)" :disabled="!selectedCategoryId"
                    class="shadcn-select-sm">
                    <option value="" x-text="!selectedCategoryId ? 'Subcategoría...' : 'Todas las Subcategorías'"></option>
    ...
    <div class="flex-1 overflow-x-auto min-h-[360px] relative">
        ...
        <div x-show="loading" class="absolute inset-0 bg-white/70 dark:bg-zinc-950/70 z-20 flex items-center justify-center">
            <svg class="animate-spin h-6 w-6 text-zinc-600 dark:text-zinc-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <template x-if="!loading && visibleItems.length === 0">
            <div class="flex flex-col items-center justify-center h-full py-16 px-4 text-center">
                <div class="h-10 w-10 rounded-full bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-400 mb-2">
        ...
        <template x-if="visibleItems.length > 0">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-zinc-100 dark:border-zinc-900 bg-zinc-50/50 dark:bg-zinc-900/20 text-[11px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                        ...
                        <tr class="group hover:bg-zinc-50/60 dark:hover:bg-zinc-900/40 transition-colors">
                                    ...
                                    <div class="inline-flex items-center justify-center">
                                        <button type="button" 
                                            ...
                                            <img :src="item.photo" :alt="item.name"
                                                ...
                                                loading="lazy" />
                                            <div class="absolute inset-0 bg-black/25 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-white drop-shadow-xs" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <td class="py-3 px-4">
                                <div class="flex flex-col">
                                    <span class="font-medium text-zinc-900 dark:text-zinc-100 leading-snug" x-text="item.name"></span>
                                    <span class="text-[10px] text-zinc-400 mt-0.5" x-text="item.sku ? 'SKU: ' + item.sku : 'Sin SKU'"></span>
                                </div>
                            </td>

                            <!-- Col 2: Unidad -->
                            <td class="py-3 px-3 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"
                            ...
                            <!-- Col 4: Opciones Dropdown -->
                            <td class="py-3 px-4 text-right">
                                <div class="relative inline-flex justify-end">
                                    <button type="button" @click.stop="activeMenuIndex = (activeMenuIndex === index ? null : index)"
                                    ...
                                    <div x-show="activeMenuIndex === index" x-cloak
                                        ...
                                        <button type="button" @click="duplicateItem(item)"
    ...
    <div class="p-3 border-t border-zinc-100 dark:border-zinc-900 bg-zinc-50/40 dark:bg-zinc-900/20 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
    ...
    <template x-teleport="body">
        <div x-show="previewImage" x-cloak
            ...
            <div class="relative max-w-sm sm:max-w-md w-full bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden"
                ...
                <div class="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-900 bg-zinc-50/50 dark:bg-zinc-900/30">
                    <div class="truncate pr-3">
                        <h4 class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 truncate" x-text="previewImage?.name"></h4>
                        <p class="text-[10px] text-zinc-400 font-mono mt-0.5" x-text="previewImage?.sku ? 'SKU: ' + previewImage.sku : 'Sin SKU'"></p>
                    </div>
                ...
                <div class="p-3 bg-zinc-100/50 dark:bg-zinc-900/50 flex items-center justify-center min-h-[220px] max-h-[75vh]">
