<div
    id="category-subcategory-modal-root"
    x-data="{
        isOpen: false,
        activeTab: 'category',
        categories: [],
...
>
    ...
    >
        ...
        >
            ...
            <!-- Shadcn Tab Switcher -->
            <div class="inline-flex h-9 w-full items-center justify-center rounded-lg bg-zinc-100 p-1 text-xs text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400 mt-4 mb-4">
                <button
                    type="button"
                    @click="activeTab = 'category'; errorMessage = ''; successMessage = '';"
                    :class="activeTab === 'category' ? 'bg-white text-zinc-950 shadow-sm dark:bg-zinc-950 dark:text-zinc-50 font-medium' : 'hover:text-zinc-900 dark:hover:text-zinc-200'"
                ...
                <button
                    type="button"
                    @click="activeTab = 'subcategory'; errorMessage = ''; successMessage = '';"
                    :class="activeTab === 'subcategory' ? 'bg-white text-zinc-950 shadow-sm dark:bg-zinc-950 dark:text-zinc-50 font-medium' : 'hover:text-zinc-900 dark:hover:text-zinc-200'"
                    class="inline-flex flex-1 items-center justify-center rounded-md py-1 transition-all"
                >
                    Nueva Subcategoría
                </button>
            ...
            <template x-if="errorMessage">
                <div class="mb-4 rounded-md border border-red-200 bg-red-50/50 p-2.5 text-xs text-red-600 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-400 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-red-500" viewBox="0 0 20 20" fill="currentColor">
            ...
            <template x-if="successMessage">
                <div class="mb-4 rounded-md border border-emerald-200 bg-emerald-50/50 p-2.5 text-xs text-emerald-600 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-400 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-emerald-500" viewBox="0 0 20 20" fill="currentColor">
            ...
            <form x-show="activeTab === 'category'" @submit.prevent="submitCategory()" class="space-y-4">
                <div class="space-y-1.5">
                    <label class="text-xs font-medium text-zinc-900 dark:text-zinc-100 flex items-center justify-between">
                        <span>Nombre de la Categoría <span class="text-red-500">*</span></span>
                    </label>
                    <input
                        type="text"
                        x-model="categoryForm.name"
                        placeholder="Ej. Electricidad, Ferretería, EPPs..."
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea
                        ...
                        class="shadcn-input !h-16 !text-xs py-2 resize-none"
                    ></textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-900">
                    ...
                    >
                        <template x-if="loadingSubmit">
                            <svg class="animate-spin -ml-0.5 mr-1.5 h-3.5 w-3.5 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <form x-show="activeTab === 'subcategory'" @submit.prevent="submitSubcategory()" class="space-y-4">
                <div class="space-y-1.5">
                    ...
                    <select
                        x-model="subcategoryForm.category_id"
                        class="shadcn-select !h-9 !text-xs"
                ...
                <div class="space-y-1.5">
                    ...
                    <input
                        type="text"
                        x-model="subcategoryForm.name"
                        placeholder="Ej. Cables, Interruptores, Guantes..."
                        class="shadcn-input !h-9 !text-xs"
                    />
                </div>
